...
export function Sidebar({ track, lessons }: SidebarProps) {
  const pathname = usePathname();
  const [completed, setCompleted] = useState<string[]>([]);
  const [collapsed, setCollapsed] = useState<Set<string>>(new Set());

  useEffect(() => {
    setCompleted(getCompletedLessons(track));
  ...
  return (
    <aside className="w-60 flex-shrink-0 bg-[#07070b] border-r border-slate-800/50 h-screen flex flex-col overflow-hidden">
      {/* Brand + track header */}
      ...
      {/* Quick nav */}
      <div className="px-2 py-2 border-b border-slate-800/50 flex-shrink-0">
        {NAV.map(({ href, label, icon }) => {
          const path = href(track);
          const isActive = pathname === path || (path.includes('/lessons') && pathname.startsWith(path) && !pathname.includes('/['));
          return (
            <Link
              key={path}
              href={path}
          ...
          return (
            <div key={category} className="mb-1">
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-[11px] font-semibold text-slate-600 hover:text-slate-400 uppercase tracking-wider transition-colors"
              >
                <span>{category}</span>
                ...
                <div className="space-y-0.5 mb-1">
                  {catLessons.map((lesson) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`w-4 h-4 rounded-full border flex-shrink-0 flex items-center justify-center text-[9px] ${
                            isDone
      ...
      <div className="px-4 py-3 border-t border-slate-800/50 flex-shrink-0">
        <Link href="/" className="text-[11px] text-slate-600 hover:text-slate-400 transition-colors flex items-center gap-1.5">
          ← All Tracks
        </Link>
      </div>
    </aside>
